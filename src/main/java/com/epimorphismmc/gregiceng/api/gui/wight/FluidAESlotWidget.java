    ...
    public void drawInForeground(
        ...
        if (isMouseOverElement(mouseX, mouseY) && getHoverElement(mouseX, mouseY) == this) {
            if (!stack.isEmpty() && gui != null) {
                ...
                tooltips.add(Component.translatable("ldlib.fluid.amount", amount, amount)
                        .append(" " + FluidHelper.getUnit()));
                tooltips.add(
                        Component.translatable("ldlib.fluid.temperature", FluidHelper.getTemperature(stack)));
                tooltips.add(Component.translatable(
                        FluidHelper.isLighterThanAir(stack)
    ...
    public void drawInBackground(
        ...
        if (!stack.isEmpty()) {
            ...
            graphics.drawString(
                    fontRenderer,
                    s,
                    (int) ((stackX + (getSizeWidth() / 3f)) * 2 - fontRenderer.width(s) + 21),
                    (int) ((stackY + (getSizeHeight() / 3f) + 6) * 2),
                    0xFFFFFF,
                    true);
